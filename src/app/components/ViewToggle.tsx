...
export function ViewToggle({ value, onChange }: ViewToggleProps) {
  return (
    <button
      type="button"
      onClick={(e) => { e.stopPropagation(); onChange(value === "grid" ? "list" : "grid"); }}
      className="relative w-20 h-10 rounded-full bg-zinc-900 cursor-pointer transition-colors border border-zinc-600/50"
    >
      <div className={`absolute top-1 w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center transition-all duration-200 ${
        value === "list" ? "left-[calc(100%-2.25rem)]" : "left-1"
      }`}>
        <span className="scale-[0.8]">{value === "grid" ? <GridIcon /> : <ListIcon />}</span>
      </div>
    </button>
  );
}
