        ...
        >
          ...
          {query && (
            <button
              type="button"
              onClick={onClear}
              className={`absolute right-4 top-1/2 -translate-y-1/2 ${isDesktop ? "w-8 h-8" : "w-7 h-7"} flex items-center justify-center rounded-md bg-zinc-700 hover:bg-zinc-600 text-zinc-400 hover:text-white transition-colors cursor-pointer`}
            >
              <CloseIcon size={isDesktop ? 18 : 16} />
            </button>
          )}
